  ...
  return (
    <>
      <Seo
        ...
        description="Completed and ongoing projects across Jaipur — Mansarovar, Jagatpura, Pratap Nagar, Sanganer, Malviya Nagar and Vaishali Nagar."
          ...
          <div className="surface sticky top-[calc(var(--nav-h)+8px)] z-20 rounded-xl border p-4 shadow-sm">
            ...
            {activeFilters.length > 0 && (
              <div className="mt-3 flex flex-wrap items-center gap-2 border-t pt-3">
                ...
                <button onClick={() => setParams({}, { replace: true })} className="text-caption text-subtle underline underline-offset-2 hover:text-cyan-700">
                  Clear all
                </button>
              </div>
            )}
              ...
              <Reveal>
                ...
                <div id="atlas-project-list" className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  {filtered.map((p) => (
                    <button
                      key={p.id}
                      onClick={() => navigate(ROUTES.project(p.slug))}
